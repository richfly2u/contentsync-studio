...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* ═══ Header ═══ */}
      <header className="border-b border-[var(--card-border)] sticky top-0 bg-[var(--nav-bg)] backdrop-blur z-50">
      ...
      <main className="flex-1">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-xs font-medium transition-all ${
                      mode === m.id
                        ...
                        : "bg-[var(--muted)] border border-[var(--card-border)] text-gray-600 dark:text-gray-400 hover:border-gray-300 dark:hover:border-gray-600"
                    }`}
                  >
              ...
              <div className="flex items-start gap-2">
                <div className="flex-1 relative">
                  <textarea
                    ...
                    className="w-full px-4 py-3 bg-[var(--input-bg)] border border-[var(--card-border)] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                    rows={2}
                    maxLength={4096}
                  />
                  {url && (
                ...
                <button onClick={handlePaste} className="px-4 py-3 border border-[var(--card-border)] rounded-xl text-xs text-gray-600 dark:text-gray-400 hover:bg-[var(--muted)] transition-colors shrink-0">
                  粘貼
                </button>
                <button
                  onClick={() => handleParse()}
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-md transition-colors ${
                      showHistory
                        ...
                        : "text-gray-400 dark:text-gray-500 hover:bg-[var(--muted)]"
                    }`}
                  >
                    最近解析 ({history.length})
                  </button>
                ...
                <div className="glass-card-strong rounded-xl overflow-hidden">
                  ...
                  <div className="bg-gray-50 dark:bg-gray-900/50">
                      ...
                      <div className="p-3">
                        ...
                        <div className="mt-2 flex gap-1.5">
                          ...
                          <button onClick={() => navigator.clipboard.writeText(result.video_url)}
                            className="px-3 text-xs border border-[var(--card-border)] rounded-lg hover:bg-[var(--muted)]">
                            複製直連
                          </button>
                        </div>
                      </div>
                    ...
                    {mode === "extract-caption" && (
                      <div className="p-3">
                        <div className="bg-[var(--input-bg)] border border-[var(--card-border)] rounded-lg p-3 max-h-48 overflow-y-auto text-xs text-gray-700 dark:text-gray-300 whitespace-pre-wrap">
                        ...
                        <div className="mt-2 flex gap-1.5 flex-wrap">
                          <button onClick={() => navigator.clipboard.writeText(result.captions_cleaned || "")}
                            className="px-3 py-1.5 text-xs border border-[var(--card-border)] rounded-lg hover:bg-[var(--muted)]">📋 複製乾淨文稿</button>
                          <button onClick={() => navigator.clipboard.writeText(result.captions_raw || "")}
                            className="px-3 py-1.5 text-xs border border-[var(--card-border)] rounded-lg hover:bg-[var(--muted)]">📋 複製原始文稿</button>
                        </div>
                      </div>
                    )}
                    {mode === "ocr" && (
                      <div className="p-3">
                        ...
                        <button onClick={() => navigator.clipboard.writeText(result.text || "")}
                          className="mt-2 px-3 py-1.5 text-xs border border-[var(--card-border)] rounded-lg hover:bg-[var(--muted)]">📋 複製文字</button>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-2 text-[10px] text-gray-400 dark:text-gray-500 mb-1.5">
                      <span className="bg-[var(--muted)] px-1.5 py-0.5 rounded">
                  ...
                  <div className="space-y-1.5 max-h-80 overflow-y-auto">
                    {history.map((item) => (
                      <button
                        ...
                        className="w-full flex items-center gap-3 p-2 rounded-lg hover:bg-[var(--muted)] text-left transition-colors"
                      >
            ...
            <div className="mt-6 pt-4 border-t border-[var(--card-border)] text-center">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {["抖音", "小紅書", "YouTube", "TikTok", "快手", "B站", "微博", "Facebook", "Instagram", "Twitter/X"].map((p) => (
                  <span key={p} className="text-[10px] text-gray-400 dark:text-gray-500 bg-[var(--muted)] px-2 py-1 rounded">{p}</span>
                ))}
              </div>
            </div>
          </div>
        ...
        <section className="py-16 max-w-5xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={f.title} className="bg-[var(--card)] border border-[var(--card-border)] p-6 rounded-xl hover:shadow-md transition-shadow">
          ...
          <div className="max-w-4xl mx-auto px-4 text-center">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {[
                ...
                <div key={s.step} className="bg-[var(--card)] p-8 rounded-xl shadow-sm">
        ...
        <section className="py-16 max-w-5xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={u.title} className="bg-[var(--muted)] p-6 rounded-xl">
          ...
          <div className="max-w-5xl mx-auto px-4">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {[
                ...
                <div key={t.title} className="bg-[var(--card)] p-6 rounded-xl border border-[var(--card-border)]">
        ...
        <section className="py-16 max-w-3xl mx-auto px-4">
          ...
          <div className="space-y-3">
            {faqItems.map((item, i) => (
              <details key={i} className="group border border-[var(--card-border)] rounded-xl overflow-hidden">
                <summary className="px-5 py-4 cursor-pointer text-sm font-medium text-gray-800 dark:text-gray-200 hover:bg-[var(--muted)] flex items-center justify-between">
                  {item.q}
      ...
      {/* ═══ Footer ═══ */}
      <footer className="border-t border-[var(--footer-border)] py-10 bg-[var(--card)]">
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-3 gap-8 text-sm">
          <div>
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-3 italic border-t border-[var(--card-border)] pt-3">
